...
!------------------------------------------------------------
	...
	include 'mpif.h'
		...
		if (myrank /= 0) then
			allocate(fixed%upper_direct(12))
			allocate(fixed%lower_direct(12))
		endif
		
		call MPI_Bcast(fixed%upper_direct,12,MPI_DOUBLE_PRECISION,0,MPI_COMM_WORLD,ierr)
		call MPI_Bcast(fixed%lower_direct,12,MPI_DOUBLE_PRECISION,0,MPI_COMM_WORLD,ierr)
