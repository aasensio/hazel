...
! Number of slabs
		...
		if (verbose_mode == 1) then
			write(*,FMT='(A,I1,A)') 'Using ', in_params%nslabs, ' slabs'
		endif

		nparam = nparam + in_params%nslabs * 2
...
!------------------------------------------------------------
	...
	integer :: i
		...
		allocate(in_fixed%upper_direct(12))
		allocate(in_fixed%lower_direct(12))
...
! Get the varid of the data variable, based on its name.
  			...
  			call check( nf90_inq_varid(in_observation%obs_id, "obs_angle", in_observation%obsangle_id) )
...
! If we are inverting a map, read the appropriate point and put it onto the arrays
		if (in_observation%observation_format == 2) then
			...
			if (pixel <= observation%npixel) then
...
! Read observation theta angle
				call check( nf90_get_var(in_observation%obs_id, in_observation%obsangle_id, values_vec,&
					start=(/ pixel /), count=(/ 1 /) ) )
...
!------------------------------------------------------------
	...
	integer :: i
		...
		write(13,FMT='(A)') '# Number of slabs'
