...
function ddread,help=help,file,formatted=formatted,unformatted=unformatted $
  ...
  ,verbose=verbose, noverbose=noverbose, double=double
;
;	=====>> HELP
;
on_error,2
...
;
nrows = 1L*(lend - lbegin + 1)
array = make_array(size=[2,ncolumns,nrows,type,ncolumns*nrows],double=double)
tmp = make_array(size=[1,ncols,type,ncols],double=double)	; READ BUFFER
