	...
	def loadConfig(self):
		if (os.path.exists("state.pickle")):
			try:
				...
				self.boundaryInput  = np.zeros((self.nLambdaInput,4))
		else:
			self.synModeInput = 5
			self.nSlabsInput = 1
			self.B1Input = np.asarray([3.0,80.0,41.0])
...
#######################################################################
	...
	def redrawProfiles(self):
		...
		[l, stokes, etaOutput, epsOutput] = pyhazel.synth(self.synModeInput, self.nSlabsInput, self.B1Input, self.B2Input, self.hInput, 
						self.tau1Input, self.tau2Input, self.boundaryInput, self.transInput, self.atomicPolInput,self.magoptInput, self.anglesInput, 
